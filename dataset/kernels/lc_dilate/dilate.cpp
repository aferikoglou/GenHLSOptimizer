...
L6:	    for (int i = 0; i < (TILE_COLS+2*MAX_RADIUS) * (2 * MAX_RADIUS) + MAX_RADIUS + PARA_FACTOR; i++) {
	        img_rf[i + MAX_RADIUS] = img[i];
	    }

L7:	    for (int i = 0; i < ((TILE_COLS+2*MAX_RADIUS) * TILE_ROWS) / PARA_FACTOR ; i++) {
...
L12:	        for (int k = 0; k < (TILE_COLS+2*MAX_RADIUS) * (2 * MAX_RADIUS) + MAX_RADIUS * 2; k++) {
...
L22:	for (int k = 0; k < GRID_ROWS / TILE_ROWS; k++) {
			load_data_tile(img_inner_0, img, k);

L23:		for (int j = 0; j < GRID_COLS / TILE_COLS; ++j){

				if (j == 0){
					start_col = 0;
L24:					for (int row=0; row<(TILE_ROWS + 2 * MAX_RADIUS); ++row){
L25:						for (int col=0; col<(TILE_COLS + MAX_RADIUS); ++col){
							img_inner_col[row*(TILE_COLS + 2 * MAX_RADIUS)+MAX_RADIUS+col] = img_inner_0[row*GRID_COLS+col];
						}
					}
L26:					for (int row=0; row<(TILE_ROWS + 2 * MAX_RADIUS); ++row){
L27:						for (int col=0; col<MAX_RADIUS; ++col){
...
L28:					for (int row=0; row<(TILE_ROWS + 2 * MAX_RADIUS); ++row){
L29:						for (int col=0; col<(TILE_COLS + MAX_RADIUS); ++col){
							img_inner_col[row*(TILE_COLS + 2 * MAX_RADIUS)+col] = img_inner_0[row*GRID_COLS+start_col+col];
						}
					}
L30:					for (int row=0; row<(TILE_ROWS + 2 * MAX_RADIUS); ++row){
L31:						for (int col=0; col<MAX_RADIUS; ++col){
					...
					start_col = j * TILE_COLS - MAX_RADIUS;
L32:					for (int row=0; row<(TILE_ROWS + 2 * MAX_RADIUS); ++row){
L33:						for (int col=0; col<(TILE_COLS + 2 * MAX_RADIUS); ++col){
							img_inner_col[row*(TILE_COLS + 2 * MAX_RADIUS)+col] = img_inner_0[row*GRID_COLS+start_col+col];
						}
					}
				}
...
L34:				for (int row=0; row<TILE_ROWS; ++row){
L35:					for (int col=0; col<TILE_COLS; ++col){
